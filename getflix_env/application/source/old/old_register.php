...
$req = $conn->prepare("SELECT * FROM register");
$req->execute();
$rows = $req->fetchAll();
...
if (isset($_POST["submit"])) {
    ...
    foreach ($rows as $row) {
        //echo $email . "<br>";
        //echo $row['email'] . "<br>";
        if ($email === $row["email"] and filter_var($email, FILTER_VALIDATE_EMAIL) and password_verify($password, $row["password"])) {
            echo "Ok => go to the main page";
            break;
        } else {
            $message_for_user = "Your info is not correct.";
            break;
        }
    }
}
